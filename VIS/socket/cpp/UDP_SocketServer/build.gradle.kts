...
tasks.register("run", Exec::class) {
    ...
    when {
        ...
        else -> { throw GradleException(":cpp:UDP_SocketServer run-target -> unknown OS family encountered")}
    }
}
...
tasks.register("kill", Exec::class) {
    ...
    when {
        ...
        else -> { throw GradleException(":cpp:UDP_SocketServer kill-target -> unknown OS family encountered")}
    }
}
