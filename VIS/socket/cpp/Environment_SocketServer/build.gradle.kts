...
tasks.register("run", Exec::class) {
    ...
    when {
        Os.isFamily(Os.FAMILY_WINDOWS) -> {
            exeFile = "Environment_SocketServer.exe"
			commandLine("cmd", "/k", exeDir+exeFile) // start command in cmd shell
        }
        Os.isFamily(Os.FAMILY_MAC) -> {
            exeFile = "Environment_SocketServer"
        ...
        else -> { throw GradleException(":cpp:Environment_SocketServer run-target -> unknown OS family encountered")}
    }
}
...
tasks.register("kill", Exec::class) {
    ...
    when {
        ...
        else -> { throw GradleException(":cpp:Environment_SocketServer kill-target -> unknown OS family encountered")}
    }
}
